...
class RotationAxisAngleChecker(BaseChecker):
    ...
    def check(self, handler: BaseSimHandler) -> torch.BoolTensor:
        ...
        log.debug(
            f"Angle between selected {self.axis} axis and target vector {self.target_vector} is: {angle_rad} in radians"
        )
...
class PositionShiftRangeChecker(BaseChecker):
    ...
    def check(self, handler: BaseSimHandler) -> torch.BoolTensor:
        ...
        return (dim_diff >= self.distance_lower) & (dim_diff <= self.distance_upper) * (
            tot_dis <= self.bounding_distance
        )
